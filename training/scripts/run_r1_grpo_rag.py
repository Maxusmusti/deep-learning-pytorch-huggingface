...
def equation_reward_func(completions, target, nums, **kwargs):
    ...
    for completion, gt, numbers in zip(completions, target, nums):
      try:
        ...
        rouge_scores = scorer.score(gt, model_answer)
        ...
        #logger.info()
        logger.info("*"*30)
        logger.info(f"\033[94mModel Response:\033[0m {model_answer}")
        logger.info(f"\033[92mGround Truth:\033[0m {gt}")
        logger.info(f"\033[93mROUGE Scores:\033[0m {rouge_scores}")
        logger.info("*"*30)
        # Extract all numbers from the equation

        #logger.info("OK LETS SEE")
        rewards.append(rouge_scores)
      except Exception:
        #logger.info("WHAT")
